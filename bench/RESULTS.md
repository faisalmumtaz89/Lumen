...
- For long-context numbers, the tiled streaming-softmax decode threshold is default-on (`ATTN_DECODE_TILED_DEFAULT_THRESHOLD = 0` in `crates/lumen-runtime/src/cuda/decode.rs`). The Qwen3.5-9B production model (head_dim=256) is eligible for the tiled family; Q8_0/BF16-body dense defaults upgrade eligible automatic selections to the split-K pair, and so does a Q4_0-body dense model on compute capability 12.x (Q4 remains Tiled on every other capability). Tiny test fixtures (head_dim<BLOCK_DIM) auto-fall-back to SingleBlock per the head_dim compatibility guard.
