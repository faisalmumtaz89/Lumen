//! Compare a component's Rust output against a reference tensor dumped by the
//! oracle, with the oracle's own inputs injected.
//!
//! Injecting the inputs is what makes a per-stage check meaningful: a component that
//! is fed the reference's input cannot hide its own error behind an upstream one.
//!
//! Usage: `dit-check <lbi-dir> <oracle-dir> [tag]`
...
fn run() -> Result<(), String> {
    ...
    let usage = || {
        eprintln!("usage: dit-check <lbi-dir> <oracle-dir> [tag]");
        std::process::exit(2)
    };
    ...
    let dit = Dit::load(&lbi_dir.join("transformer.lbi")).map_err(|e| format!("{e}"))?;

    // The oracle's own step-0 inputs.
    ...
    let hidden = lumen_image::tensor::Matrix::new(seq, ch, hs.data[..seq * ch].to_vec());
    ...
    let encoder = lumen_image::tensor::Matrix::new(ehs.shape[1], ehs.shape[2], ehs.data.clone());
    ...
    println!(
        "dit prefill   rel-L2={:.3e}  max_abs={:.3e}  cosine={:.8}",
