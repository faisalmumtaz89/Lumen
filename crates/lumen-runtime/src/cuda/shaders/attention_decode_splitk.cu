...
// ---------------------------------------------------------------------------
// 16-bit K/V twin of the partial pass. The same kernel with K and V read as
// IEEE half and widened on load (exact); every operation after the load is the
// F32 kernel's in the F32 kernel's order, so on half-representable inputs the
// two write bit-identical partials, and the F32 merge above consumes them
// unchanged. Kept in this file so the two cannot drift apart unnoticed.
// ---------------------------------------------------------------------------

__device__ __forceinline__ float splitk_h2f(unsigned short h) {
    float f;
    asm("cvt.f32.f16 %0, %1;" : "=f"(f) : "h"(h));
    return f;
}

__device__ __forceinline__ float splitk_qk_dot_f16(
    const float* __restrict__ q_row, const unsigned short* __restrict__ k_vec,
    unsigned int head_dim, float scale)
{
    float dot = 0.0f;
    if ((head_dim & 3u) == 0u) {
        ...
        for (unsigned int d4 = 0; d4 < hd4; d4++) {
            ...
            uint2 r = k2[d4];
            float4 k;
            k.x = splitk_h2f((unsigned short)(r.x & 0xffffu));
            k.y = splitk_h2f((unsigned short)(r.x >> 16));
            k.z = splitk_h2f((unsigned short)(r.y & 0xffffu));
            k.w = splitk_h2f((unsigned short)(r.y >> 16));
            dot += q.x * k.x + q.y * k.y + q.z * k.z + q.w * k.w;
        }
    } else {
        for (unsigned int d = 0; d < head_dim; d++) {
            dot += q_row[d] * splitk_h2f(k_vec[d]);
        }
    }
    return dot * scale;
}

extern "C" __global__ void attention_decode_splitk_partial_f16(
    const float* __restrict__ q,           // [num_heads * head_dim]
    ...
    if (p0 < p1) {
        ...
        for (unsigned int tile = 0; tile < num_tiles; tile++) {
            ...
            for (unsigned int j = tid; j < T_C; j += block_size) {
                if (j < tile_len) {
                    unsigned int pos = tile_start + j;
                    const unsigned short* k_vec =
                        k_cache + kv_base + (unsigned long long)pos * (unsigned long long)head_dim;
                    s_tile[j] = splitk_qk_dot_f16(q_row, k_vec, head_dim, scale);
                } else {
                    s_tile[j] = NEG_INF;
                }
            }
...
#pragma unroll
            for (unsigned int slot = 0; slot < MAX_SLOTS; slot++) {
                ...
                if (d < head_dim) {
                    ...
                    for (unsigned int j = 0; j < tile_len; j++) {
                        unsigned int pos = tile_start + j;
                        float v_dj = splitk_h2f(v_cache[kv_base
                            + (unsigned long long)pos * (unsigned long long)head_dim
                            + (unsigned long long)d]);
                        pv += s_tile[j] * v_dj;
                    }
