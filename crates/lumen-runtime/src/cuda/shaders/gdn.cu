...
// ============================================================================
// gdn_ab_p123_fused: source-fidelity glue bundle (LUMEN_CUDA_GDN_GLUE).
//
// Concatenates the banked F32 alpha/beta gate projections (96 row-CTAs, with
// the gate TRANSFORMS moved into the row epilogue) and the p123 conv kernel
// (80 CTAs, its former tid==0 gates block removed — the dependency the
// transforms created) into ONE 176-CTA launch. Every arithmetic expression
// is a verbatim clone of its source kernel (matvec_f32_gates_banked's fmaf
// dot + fixed reduction; gdn_decode_phase123_fused's softplus/exp/sigmoid
// and conv/L2 bodies), so outputs are bit-identical to the two-launch chain.
//
...
extern "C" __global__ void gdn_ab_p123_fused(
    // gates part
    const float* __restrict__ w_alpha,   // [num_v_heads, in_dim] F32
    const float* __restrict__ w_beta,    // [num_v_heads, in_dim] F32
    const float* __restrict__ normed,    // [in_dim] F32 activation
    const float* __restrict__ dt_bias,   // [num_v_heads]
    const float* __restrict__ ssm_a,     // [num_v_heads]
    float* __restrict__ alpha_out,       // [num_v_heads] TRANSFORMED
    float* __restrict__ beta_out,        // [num_v_heads] TRANSFORMED
    unsigned int in_dim,
    // p123 part
    const float* __restrict__ input,     // [qkv_dim]
...
{
    __shared__ float shared[33];
    const unsigned int gate_ctas = 2u * num_v_heads;

    if (blockIdx.x < gate_ctas) {
        // ---- gates row (verbatim matvec_f32_gates_banked body) ----
        const unsigned int is_beta = blockIdx.x >= num_v_heads;
        const unsigned int row = is_beta ? (blockIdx.x - num_v_heads) : blockIdx.x;
        const float* w = (is_beta ? w_beta : w_alpha)
            + (unsigned long long)row * in_dim;
        float acc = 0.0f;
        for (unsigned int i = threadIdx.x; i < in_dim; i += blockDim.x) {
            acc = fmaf(w[i], normed[i], acc);
        }
        ...
        if (threadIdx.x == 0) {
            ...
            for (unsigned int w2 = 0; w2 < nw; w2++) total += shared[w2];
            // ---- transform epilogue (verbatim p123 phase-2 expressions) ----
            if (is_beta) {
                beta_out[row] = 1.0f / (1.0f + expf(-total));
            } else {
                float sp_input = total + dt_bias[row];
                float sp;
                if (sp_input > 20.0f) {
                    sp = sp_input;
                } else {
                ...
                float gate = ssm_a[row] * sp;
                alpha_out[row] = expf(gate);
            }
        }
        return;
    }

    // ---- p123 CTA (verbatim gdn_decode_phase123_fused minus gates) ----
    unsigned int b = blockIdx.x - gate_ctas;
    unsigned int tid = threadIdx.x;
    if (tid >= head_dim) return;
    ...
    if (b < num_kv_heads) {
        ...
        c = qk_dim + (b - num_kv_heads) * head_dim + tid;
        is_qk = 1;
    } else {
        c = 2u * qk_dim + (b - 2u * num_kv_heads) * head_dim + tid;
        is_qk = 0;
    }
