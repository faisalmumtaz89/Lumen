...
#define Q8_1_BYTES       36    // Q8_1 activation block: [f16 d][f16 sum][32 int8]
...
//
// WHY THIS EXISTS, and why it is scoped to four tensors.
//
// The F32-activation sibling above is compute-bound: with F32 activations there
// is one `fma` per element and no way to fold four multiply-accumulates into one
// instruction, so on the head shape it lands near the Q8_0 dp4a path it would
// have to beat. dp4a changes that ratio -- one `dp4a.s32.s32` does four int8
// MACs -- which is the only way a Q6_K matvec competes with an int8 route.
//
// It is deliberately NOT wired to the head. It is wired to the FOUR coupled
// `wq` surfaces (Qwen3.5-9B-Q4_0 full-attention layers 3/15/27/31), because
// those are the sites where an int8 activation plan already pre-quantizes the
// activation into `scratch.input_q8_1` -- the Q8_1 buffer this kernel consumes
// exists there for free. Under such a plan those four layers currently divert to
// F16 HGEMV at 2.0 B/weight (the coupling), so this replaces a 2.0 B/w read with
// a 0.8203 B/w one on the exact tensors that were losing the most.
//
// ACTIVATION SEMANTICS: `vec_dot_q6_K_q8_1` (ggml/src/ggml-cuda/vecdotq.cuh:620-644
// @ 3b53219), transplanted onto this file's lane mapping rather than llama's
// `iqs` striding. Per 4-byte group: mask the nibbles, splice the two high bits,
// apply the -32 bias to all four bytes with one `__vsubss4`, `dp4a` against four
// int8 activations, scale by the int8 sub-block scale, then by the Q8_1 block's
// f32 scale. `d` is applied once per super-block.
//
// Note Q6_K does NOT use the Q8_1 `sum` field. Q4_0/Q4_1 need it to correct
// their weight offset, but Q6_K's -32 is applied per-element before the dot, so
// the header's second f16 is read by nobody here. That asymmetry is easy to
// "fix" by mistake; llama.cpp's impl has the same shape (no `bsums` term).
//
// LAYOUTS
//   Q6_K weight: as the header of this file. Lane L owns `ql` bytes 4L..4L+3.
//   Q8_1 input:  36 bytes per 32 elements -- [f16 d][f16 sum][32 x int8], quants
//                at offset +4. A lane's four elements are 4-byte aligned inside
//                that stream because `elem_lo % 4 == 0` and 36 % 4 == 0, so the
//                activation read is one aligned `int`.
//
// NUMERICS: the integer dot is exact and order-independent; every FP op is
...
// Requires: SM 6.1+ (dp4a) -- loaded via load_fn_sm80_fast_math, NOT plain
// load_fn. NVRTC's default arch is below sm_61 and would silently reject the
// `dp4a.s32.s32` opcode, leaving the handle None and the flag inert.
// ==========================================================================
