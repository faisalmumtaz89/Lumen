    ...
    fn compute_layer_gpu(
                ...
                .is_some_and(|b| b.len() >= inter_dim.div_ceil(32) * 36)
        {
            ...
            let use_split_down = (st.kernels.use_q8_split_dispatch && lw.q8_split_w_down.is_some())
                || (st.kernels.use_q4_split_dispatch && lw.q4_split_w_down.is_some());
            // Raw Q8_0/Q4_0 weights without a split sibling: the residual sibling
            // of `launch_matvec` exists for both and ends in the same single f32
            // add `residual_add` performs (`total + residual`, no product to
            // contract), so folding the residual into the store keeps output
            // bytes unchanged while eliding `residual_add` and the layer-commit
            // D2D. On a card where the split clones do not fit, every layer
            // takes this route.
            let direct_resid_raw = crate::runtime_defaults::ffn_direct_residual()
                && matches!(lw.w_down, GpuWeightBuf::Q8Raw(_) | GpuWeightBuf::Q4Raw(_));
            let note_direct_residual_raw = || {
                ...
                MARK.get_or_init(|| {
                    if super::decode::cuda_verbose() {
                        eprintln!(
                            "[CUDA] FFN_DIRECT_RESIDUAL: sep-swiglu raw down folds residual -> x_gpu \
                             (residual_add + layer-commit D2D elided)"
                        );
                    }
                });
            };
