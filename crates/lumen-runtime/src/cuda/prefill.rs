...
impl SplitKGqa6Exclusion {
    ...
    /// and `every_way_out_of_the_route_has_its_own_reason` catches one that
    /// has fallen behind in any other way.
    const ALL: [Self; 8] = [
        Self::ForcedTiled,
...
/// happened. Which route does serve the token depends on the reason (the tiled
/// kernel for a short context, the single-block kernel under a threshold
/// opt-out, the per-query-head pair otherwise), and it announces itself on its
/// own line.
