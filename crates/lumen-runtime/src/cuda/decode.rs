...
pub(crate) fn compile_all_kernels(device: &CudaDevice) -> Result<KernelSet, RuntimeError> {
    ...
    let kernels = KernelSet {
        ...
        matvec_q4_split_f32_lane: match load_fn(
            ...
            Ok(f) => {
                cuda_log!("[CUDA] matvec_q4_split_f32_lane: OK");
                Some(f)
            }
            Err(e) => {
                cuda_log!("[CUDA] matvec_q4_split_f32_lane: FAILED: {e}");
                None
            }
        },
        ...
        matvec_q4_split_f32_lane_residual: match load_fn(
            ...
            Ok(f) => {
                cuda_log!("[CUDA] matvec_q4_split_f32_lane_residual: OK");
                Some(f)
            }
            Err(e) => {
                cuda_log!("[CUDA] matvec_q4_split_f32_lane_residual: FAILED: {e}");
                None
            }
        },
