...
/// whose row width is not whole superblocks, an embedding whose element count is not whole
/// superblocks — is converted as 0.31.0 converted it): CUDA serves each preserved plane
/// through its own scheme's arm, and a missing kernel group is then reported at the first
/// token instead of at load.
