...
#[cfg(test)]
use lumen_format::serving_rules::dense_slice_quant_supported;
...
pub(crate) use lumen_format::serving_rules::AttnDims;

/// Thin wrappers over the shared serving rules in `lumen_format` — the
/// single source of truth also enforced at convert time (see
/// serving_rules.rs there). Kept as the Metal-local entry points so call
/// sites and the test suite are unchanged.
pub(super) fn validate_layer_quants(
    layer: usize,
    st: &lumen_format::index::SubtensorOffsets,
) -> Result<(), RuntimeError> {
    lumen_format::serving_rules::validate_layer_quants(layer, st).map_err(RuntimeError::Compute)
}
...
pub(super) fn validate_attention_dims(
    ...
    lumen_format::serving_rules::validate_attention_dims(layer, st, d)
...
mod tests {
    ...
    fn attention_dims_gdn_layers() {
        ...
        // conv1d must be exactly qkv_rows x conv_kernel F32s (128*4*4).
        ...
        let err = validate_attention_dims(0, &st, &d).unwrap_err();
        assert!(err.to_string().contains("ssm_conv1d is 256 bytes"), "{err}");
        st.ssm_conv1d = Some(TensorSlice {
            offset: 0,
            length: 128 * 4 * 4,
            quant: QuantScheme::F32,
