...
/// Measurement-only queued-burst probe (CUDA graph-replay campaign P0).
///
/// Quantifies how much of the per-token wall clock is exposed to host-side
/// launch scheduling: `E = W - C`, where `W` is the per-token wall of the
/// production greedy loop (one synchronize + 4-byte readback per token) and
/// `C` is the per-token makespan of the SAME kernel sequence enqueued as one
/// M-deep queue with a single terminal synchronize (inputs pre-recorded, so
/// no readback is needed). Every arm starts from an identical state: fresh KV
/// + recurrent-state reset + full prefill + `warm` normal decode steps
/// re-driven with the recorded token stream.
///
/// Gated behind `LUMEN_CUDA_BURST_PROBE=1`; never runs in production.
    ...
    pub fn burst_probe(
        ...
        if !caps.batched_prefill {
            return Err(RuntimeError::Compute(
                "burst_probe requires a batched-prefill backend".into(),
            ));
        }
        let fresh_kv = || -> Result<crate::kv::KvCache, RuntimeError> {
        ...
        // Prefill + final projection, shared by every phase (temp-0 first
        // token = plain argmax, so all arms share one token stream).
        let prefill_first = |kv: &mut crate::kv::KvCache| -> Result<u32, RuntimeError> {
            backend.reset_recurrent_state();
            let last_hidden = backend.prefill(prompt_tokens, weights, kv)?;
            let mut x = crate::compute::ActivationBuffer::zeros(
                last_hidden.len(),
                crate::compute::ComputeDtype::F32,
            );
            x.write_f32_from(&last_hidden);
            let logits = backend.compute_final(&x)?;
            Ok(logits.argmax() as u32)
        };

        // ---- Phase A: record the token stream + production W over the window.
        let total = warm + m;
        let mut inputs: Vec<u32> = Vec::with_capacity(total + 1);
        let mut w_ns: Vec<u128> = Vec::with_capacity(m);
        {
            let mut kv = fresh_kv()?;
            let mut next = prefill_first(&mut kv)?;
            inputs.push(next);
            for i in 0..total {
                let t0 = Instant::now();
                next = backend.decode_token_greedy(next, weights, &mut kv)?;
        ...
        // ---- Phase B: interleaved arms from identical state.
        // coarse: enqueue M steps, one terminal synchronize.
        // perpass: synchronize after every step (per-launch overhead bound).
        let mut run_arm = |mode: &str, mm: usize| -> Result<(f64, f64), RuntimeError> {
            let mut kv = fresh_kv()?;
            let _ = prefill_first(&mut kv)?;
            for i in 0..warm {
                let _ = backend.decode_token_greedy(inputs[i], weights, &mut kv)?;
            }
            backend.backend_synchronize()?;
            let t0 = Instant::now();
            for j in 0..mm {
                backend.decode_token_greedy_enqueue(inputs[warm + j], weights, &mut kv)?;
                if mode == "perpass" {
                    backend.backend_synchronize()?;
                }
            }
            ...
            Ok((total_ms / mm as f64, issue_ms / mm as f64))
        };

        let mut coarse: Vec<f64> = Vec::new();
        let mut coarse_issue: Vec<f64> = Vec::new();
        let mut perpass: Vec<f64> = Vec::new();
        let mut coarse_half: Vec<f64> = Vec::new();
        for _ in 0..4 {
            let (c, ci) = run_arm("coarse", m)?;
            coarse.push(c);
            coarse_issue.push(ci);
            let (p, _) = run_arm("perpass", m)?;
            perpass.push(p);
        }
        for _ in 0..2 {
            let (c, _) = run_arm("coarse", m / 2)?;
            coarse_half.push(c);
        }
        ...
        let mut w_ms: Vec<f64> = w_ns.iter().map(|&n| n as f64 / 1e6).collect();
        let w = med(&mut w_ms);
        let c = med(&mut coarse);
        let p = med(&mut perpass);
        let ch = med(&mut coarse_half);
        let ci = med(&mut coarse_issue);
        Ok(format!(
            "{{\"probe\":\"burst_exposure\",\"warm\":{warm},\"m\":{m},\
\"w_ms\":{w:.5},\"coarse_ms\":{c:.5},\"exposure_ms\":{e:.5},\
\"perpass_ms\":{p:.5},\"coarse_half_ms\":{ch:.5},\"cpu_issue_ms\":{ci:.5},\
\"busy_fraction\":{bf:.5},\"w_samples_ms\":{ws:?},\"coarse_samples_ms\":{cs:?}}}",
            e = w - c,
            bf = c / w,
            ws = w_ms,
            cs = coarse,
