//! Microbenchmark: NR8 (NR=8) Q8 split matvec vs production split (NR=2)
//! and 4THREAD (NR=2) Q8 split matvec across Qwen3.5-9B production shapes.
//!
//! Times all three kernels via cudaEvent. Per-shape median of NUM_TRIALS
//! measurements after NUM_WARMUP warmup iterations.
//!
//! `#[ignore]`-gated so it doesn't run in CI but can be invoked on Modal:
//!
//!   cargo test --release -p lumen-runtime --features cuda \
//!       --test cuda_q8_nr8_microbench -- --ignored --nocapture
//!
//! Output (parseable):
//!   [MICROBENCH] shape=4096x4096   split=NN.NNNus  4thread=NN.NNNus  nr8=NN.NNNus  full/split=N.NNNx  full/style=N.NNNx
...
struct Setup {
    ...
    style_fn: cudarc::driver::CudaFunction,
    full_fn: cudarc::driver::CudaFunction,
...
fn build_setup(out_dim: usize, in_dim: usize, seed: u64) -> Setup {
    ...
    let style_src = lumen_runtime::cuda::shaders::MATVEC_Q8_SPLIT_Q8_1_4THREAD_KERNEL_SOURCE;
    let full_src = lumen_runtime::cuda::shaders::MATVEC_Q8_SPLIT_Q8_1_NR8_KERNEL_SOURCE;
    ...
    let split_fn = split_module
        ...
        .expect("split fn");
    let style_fn = style_module
        .load_function("matvec_q8_split_q8_1_4thread")
        .expect("style fn");
    let full_fn = full_module
...
unsafe fn launch_style(s: &mut Setup) {
    let out_dim_u32 = s.out_dim as u32;
    let in_dim_u32 = s.in_dim as u32;
    let grid = (out_dim_u32 + 1) / 2; // NR=2
    let cfg = LaunchConfig {
...
unsafe fn launch_full(s: &mut Setup) {
    let out_dim_u32 = s.out_dim as u32;
    let in_dim_u32 = s.in_dim as u32;
...
fn bench_shape(name: &str, out_dim: usize, in_dim: usize) {
    ...
    let mut times_style = time_kernel(&mut s, |s| unsafe { launch_style(s) });
    let mut times_full = time_kernel(&mut s, |s| unsafe { launch_full(s) });
    let med_split = median(&mut times_split) * 1000.0; // ms -> us
    let med_style = median(&mut times_style) * 1000.0;
    let med_full = median(&mut times_full) * 1000.0;
    let full_vs_split = med_split / med_full;
    let full_vs_style = med_style / med_full;
    println!(
        "[MICROBENCH] shape={name:<13} out={out_dim:<7} in={in_dim:<7} split={:8.3}us  4thread={:8.3}us  nr8={:8.3}us  full/split={:.3}x  full/style={:.3}x",
        med_split, med_style, med_full, full_vs_split, full_vs_style,
    );
}
