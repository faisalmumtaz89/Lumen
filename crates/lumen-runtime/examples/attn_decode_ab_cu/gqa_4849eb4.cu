// VENDORED VERBATIM from Lumen commit 4849eb4 (branch perf/attn-splitk-gqa).
// Reproduced byte-for-byte by `git show <commit>:crates/lumen-runtime/src/cuda/shaders/attention_decode_splitk.cu`
// so the A/B harness compiles the historical kernel `attention_decode_splitk_partial_gqa`
