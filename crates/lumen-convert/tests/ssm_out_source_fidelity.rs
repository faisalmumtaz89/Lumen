...
//!
//! `ssm_out_keeps_source` has two arms: the K-quant source default, which
//! `kquant_ssm_out_geometry.rs` covers, and this switch, which answers for a source the
//! default policy never touches. Without the switch such a plane is requantised to the
//! Q8_0 floor; with it the source bytes are carried. Both halves run here, so the
//! switch is what the difference is attributed to.
//!
