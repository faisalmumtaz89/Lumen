...
impl<'a> Importer<'a> {
    ...
    /// logical shape `[n, k]`. `None` when the module is not NVFP4 —
    /// `weight_scale_2` is what distinguishes it: the FP8 modules carry a
    /// `weight_scale` too, but no second scale.
