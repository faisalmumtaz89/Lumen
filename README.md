...
> **Status:** Production-ready for the shipped Qwen3.5 / Qwen3.6 / Qwen3.8 models (dense 9B, dense 27B, and MoE-35B-A3B) on NVIDIA CUDA (compute capability 8.0+) and Apple Silicon (M-series). The public API and the binary `.lbc` format are not yet stable — **read [Production deployment](docs/production.md) before deploying.**
