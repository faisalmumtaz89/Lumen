...
10. **LBC format compatibility**: `LBC_VERSION = 4`; an artifact whose embedding is an as-stored K-quant plane is version 5 (the default generic conversion of a K-quant source whose `token_embd` is Q4_K, Q5_K or Q6_K produces one — both registry cells; `--dequantize` leaves no such plane, and that artifact keeps version 4), which a Lumen before this release refuses with `UnsupportedVersion`. The reader rejects anything newer than version 5 with `UnsupportedVersion`; backward-compat for v1/v2 is in the code path but unverified at runtime. **Policy: rebuild LBCs after major Lumen upgrades** via `lumen convert` or `lumen pull --quant <scheme>`.
