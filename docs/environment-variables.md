...
| `LUMEN_CUDA_KQUANT` | ON | kill-switch | The general Q4_K/Q5_K/Q6_K kernels that serve a K-quant artifact's planes natively (`Q4_K_M`, `Q5_K_M` sources): decode matvec, prefill dequant tile and embedding gather per scheme. `=0` refuses at load a K-quant artifact that carries a Q4_K/Q5_K/Q6_K layer or embedding plane — the tensor, its scheme and the backend are named — instead of serving it through the F32 host-dequant fallback, which cannot hold a 27B on 32 GB. A Q4_0/Q8_0/BF16 artifact's occasional K-quant plane is unaffected. | `=0` only to roll back to the load refusal; there is no pre-kernel serving path to A/B against. |
