...
| `LUMEN_CUDA_Q8_SPLIT_KEEP_RAW` | OFF (raw planes released on K-quant artifacts) | kill-switch | On a K-quant artifact (`Q4_K_M`, `Q5_K_M`), once a Q8_0 plane has its split sibling the raw copy is released and the plane is resident once, in the split layout; the prefill dequantizes that layout (a tile bit-identical to the raw dequant) and the decode fallbacks quantize the activation and run the split kernel. Saves the clone's bytes (0.87 GB on the 27B Q4_K_M at 4,096 context). The Q4_0 / Q8_0 / BF16 artifacts keep both copies as shipped, and the release skips itself when the prefill reads the raw bytes directly (`LUMEN_CUDA_Q8_PROJ_MMQ` set, a MoE model, the conv-state parity reprojection) or when the split-only route's kernels or activation scratch did not load. | `=1` to keep both copies on K-quant artifacts too. |
...
| `LUMEN_CUDA_VERBOSE` | OFF | diagnostic | Verbose CUDA backend logging (kernel-load failures, and a one-line `ACTIVE` announcement from each decode-attention, dense decode-matvec and output-head route the first time it is dispatched, naming the CUDA symbol that launched, or the cuBLAS entry point where cuBLAS chose the kernel; MoE expert dispatches do not announce). The `[CUDA] K-quant planes:` counters line prints at load on a K-quant artifact (`Q4_K_M`, `Q5_K_M`) regardless, and under `=1` on any artifact. | Diagnose kernel-load / driver issues; confirm which decode routes a run actually took. |
