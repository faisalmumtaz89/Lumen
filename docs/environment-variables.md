...
| `LUMEN_CUDA_ATTN_SPLITK_GQA` | OFF | experiment | With the split-K pair selected, one block per (KV head, chunk) serves every query head of that KV head's GQA group from a single read of K and V, on small chunks, instead of one block per query head re-reading them; needs query heads a multiple of KV heads with at most 8 per group, a head_dim that is a multiple of 128, and a chunk within one 128-position tile. Summation order differs from the per-query-head pass (a near-tie numerics change). | Set to `1` to measure the GQA-group pass at context. |
| `LUMEN_CUDA_ATTN_SPLITK_GQA_CHUNK` | 64 | config | KV positions per block for the GQA-group pass (32 to 128); the split count is the context divided by this, capped at the scratch bound. | Sweep when tuning that pass at context. |
