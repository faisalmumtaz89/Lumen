...
| `LUMEN_CUDA_ATTN_SPLITK_WARP` | ON | kill-switch | With the split-K pair selected, the partial pass gives a warp to each KV position (a contiguous K row per warp, a warp tree for the dot) and unrolls the value pass four positions deep, instead of one lane per position walking the head serially. Summation order differs. | Set to `0` for the lane-per-position partial pass. |
