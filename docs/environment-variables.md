...
| `LUMEN_CUDA_ATTN_SPLITK_CODEGEN` | unset (NVRTC default target) | experiment | Compile the split-K decode-attention pair for an explicit NVRTC target (`ptx80` / `ptx120`); same source and math options, different generated code (NVRTC 13.3: 4264 instructions at its default target vs 3496 at compute_120). The dispatch announces `attention_decode_splitk_partial_ptx120` with `merge=…_ptx120`. | `=ptx120` / `=ptx80` to A/B the code generation. |
...
| `LUMEN_CUDA_NORM_CTA5_DUAL` | ON for a Q4_0 dense body on compute capability 12.x (measured on 12.0: source-fidelity Qwen3.8-27B Q4_0 on the RTX 5090, +4.10 % decode; every 12.x part takes it); OFF elsewhere (unmeasured) | kill-switch | One multi-CTA launch at the GDN input norm writes both the normalized F32 vector and the Q8_1 blocks (replacing the plain rmsnorm + fused pair; implies the CTA5 route at every fused norm site); both outputs byte-identical (device twin tests: the dual kernel at dims 2048/4096/5120/5152, the quantization-only kernel at 2048/4096/5120). The dual launch fires only where the GDN alpha/beta projections read the F32 vector (F32 `ssm_alpha`/`ssm_beta`, as in the source-fidelity 27B artifact, or the MoE F16 path); elsewhere the flag is the CTA5 route alone. Follows `LUMEN_CUDA_LEGACY_DEFAULTS`. | `=0` to A/B the two-launch pair on Blackwell; `=1` to force it on an unmeasured capability. |
| `LUMEN_CUDA_Q4_RAW_EXACTK` | OFF | experiment | Raw Q4_0 dp4a matvec launched with 160 threads at K=5120 (the three warps that own no block there dropped), output byte-identical; other K unchanged. | `=1` to A/B against the 256-thread launch. |
| `LUMEN_CUDA_RMSNORM_Q8_CTA5` | OFF on its own; implied ON wherever `LUMEN_CUDA_NORM_CTA5_DUAL` resolves on | experiment | Fused RMSNorm+Q8_1 quantization over ceil(dim/32 ÷ warps) CTAs (5 at dim 5120) instead of one block; each CTA repeats the reduction, output byte-identical. | `=1` to A/B against the single-block kernel where the dual route is off. |
...
| `LUMEN_CUDA_ATTN_PRECISE` | numeric, model-aware: `3` (exact-F32 QK^T **and** exact-F32 P@V) for every supported production class (MoE + dense 9B/27B × Q4/Q8/BF16); `0` (legacy F16 WMMA) only for unset/legacy callers | config | Batch-≥16 WMMA prefill full-attention precision. `0`=F16 QK^T + F16 P@V (legacy); `1`=qkf32 (exact QK^T, F16 P@V — closes the QK^T carrier but REOPENS the P@V hole, regresses GQ-014); `2`=pvf32 (F16 QK^T, exact P@V — heals GQ-014 but leaves the F16-QK carrier that flips case-08); `3`=exact F32 (both, the ratified default — heals both), computed by the tiled SGEMM path when `LUMEN_CUDA_ATTN_PREFILL_SGEMM` is on and its softmax kernel built, otherwise by the one-warp-per-row scalar kernel; `4`=split (hi/lo tensor-core approx, unqualified). | Force `1` (qkf32) or `2` (pvf32) to A/B the two carriers in isolation; `3` is the ratified correctness default. To A/B the two exact-F32 evaluation orders against each other, hold `3` and flip `LUMEN_CUDA_ATTN_PREFILL_SGEMM`. |
| `LUMEN_CUDA_ATTN_PREFILL_SGEMM` | ON (unset follows `LUMEN_CUDA_LEGACY_DEFAULTS`) | kill-switch | How `LUMEN_CUDA_ATTN_PRECISE=3` is evaluated for prefills of 16 tokens or more on the fused Q+gate full-attention layers: two strided-batched cuBLAS F32 SGEMMs (Q·Kᵀ over each KV head's query group, then P·V) around an exact-F32 causal row softmax, in query blocks of at most 512 rows. Exact F32 like the scalar kernel it replaces (no F16 carrier), in a different evaluation order, so greedy output can differ at a near-tie. It needs one score block of `query_heads_per_kv_head × min(prompt_tokens, 512) × total_kv_positions × 4` bytes — 100 MB for a 9B 12k-token prompt, 1.07 GB at 128k — sized and allocated once per prefill before the layer loop and freed when the prefill returns; that allocation is outside the load-time fit check. With `=0`, `LUMEN_CUDA_FORCE_SCALAR_ATTN=1`, a shorter prefill, a refused score-block allocation, or a softmax kernel that did not build, mode 3 runs on the scalar kernel instead; a `LUMEN_CUDA_ATTN_PRECISE` other than `3` keeps that mode's own kernel. The route does not need the WMMA kernels. | Set to `0` to bisect prefill numerics against the scalar kernel, or on a card where the score block does not fit. |
