...
| `LUMEN_CUDA_ATTN_CODEGEN` | `default` (NVRTC's default target) | A/B | The NVRTC target the decode-attention kernel is compiled for: `default`, `ptx80` (compute_80) or `ptx120` (compute_120). Same source and math options; the generated code differs. A target the toolkit or driver refuses falls back to the default target, announced at load; the `[CUDA] decode attention: compiled for ...` load line, the `ACTIVE` route line (suffixed `_ptx120` / `_ptx80`) and the `LUMEN_CUDA_ATTN_DUMP` header record which one is in force. | To A/B a device target against NVRTC's default on a new card before changing anything. |
...
| `LUMEN_CUDA_Q8_SPLIT_KEEP_RAW` | OFF (raw planes released on K-quant artifacts) | kill-switch | On an artifact whose header scheme is K-quant and that carries an as-stored K-quant plane (`Q4_K_M`, `Q5_K_M` on the generic target), once a Q8_0 plane has its split sibling the raw copy is released and the plane is resident once, in the split layout; the prefill dequantizes that layout (a tile bit-identical to the raw dequant) and the decode fallbacks quantize the activation and run the split kernel. Saves the clone's bytes (0.86 GB on the 27B Q4_K_M at 4,096 context). An artifact whose planes are Q4_0 / Q8_0 / BF16 keeps both copies as shipped — including one whose header scheme is K-quant but whose planes are not, which is what a `--target metal` conversion of a K-quant source produces, and one that keeps a K-quant plane under a requantised header — and the release skips itself when the prefill reads the raw bytes directly (`LUMEN_CUDA_Q8_PROJ_MMQ` set, a MoE model, the conv-state parity reprojection) or when the split-only route's kernels or activation scratch did not load. | `=1` to keep both copies on K-quant artifacts too. |
| `LUMEN_CUDA_KQUANT` | ON | kill-switch | The general Q4_K/Q5_K/Q6_K kernels that serve a K-quant artifact's planes natively (`Q4_K_M`, `Q5_K_M` sources): decode matvec, prefill dequant tile and embedding gather per scheme. `=0` refuses at load the Q4_K/Q5_K/Q6_K layer planes of an artifact whose header scheme is K-quant, and a Q4_K/Q5_K/Q6_K embedding plane whatever the header carries — the gather reads the plane's own scheme, and `--requant q8_0` of a K-quant source preserves such an embedding under a Q8_0 header. The tensor, its scheme and the backend are named; those kernels are such a plane's only route on this backend, and the F32 host-dequant catch-all that serves a K-quant plane elsewhere could not hold a 27B on 32 GB. A Q4_0/Q8_0/BF16 artifact's occasional K-quant layer plane is unaffected. | `=0` only to roll back to the load refusal; there is no pre-kernel serving path to A/B against. |
| `LUMEN_CUDA_Q6K_HEAD` | ON | kill-switch | Serves a preserved Q6_K output head from the four aligned planes the upload splits its 210-byte superblocks into (ql / qh / sc / d), read by the `matvec_q6k_head` decode kernel. `=0` skips the plane build and uploads a host F32 dequant of the same head instead (~5 GB for the 27B head, F32 SGEMV dispatch). Read once per process. Left ON, a K-quant artifact with a Q6_K head whose `matvec_q6k_head` kernel did not load on the device is refused at load rather than at the first token, and the refusal names this variable. | `=0` to bisect the head planes against the F32 dequant, at that VRAM cost. |
...
| Variable | Default | Category | Effect | When to touch |
|---|---|---|---|---|
| `LUMEN_CUDA_ATTN_ONE_TILE` | 704 ÷ the model's KV-head count, rounded up: 176 at 4 KV heads (Qwen3.5-9B, Qwen3.8-27B), 352 at 2 (Qwen3.5-MoE-35B-A3B) | config | The tile count up to which the decode-attention kernel runs one 16-key tile per CTA; above it the CTAs walk whole tiles at `LUMEN_CUDA_ATTN_TARGET`. The constant is a CTA budget, 704 one-tile CTAs across the KV heads, so a model whose KV-head count divides it crosses to whole tiles at the same grid size (rounding up costs a KV head at most one extra tile otherwise). On the RTX 5090 at 4 KV heads the loop is never slower than one tile per CTA below 176 tiles and faster above on the F32 store, and 176 against 256 is a wash on the half store; one bound on either store gives both the same split geometry at every context, so on half-representable inputs the half store's output is bit-identical to the F32 store's. Clamped to `1..=1024`; `0` or unparsable is the default. | Retune on another GPU with the standalone harness before changing. |
| `LUMEN_CUDA_ATTN_TARGET` | 128 on every model | config | The split count the kernel holds above the one-tile bound; each CTA walks `ceil(tiles / target)` or one fewer whole tiles, so the scratch is sized once for `max(one_tile, target)` chunks and never for the context. At 4 KV heads on the RTX 5090, 128 is within 3 % of the best measured count at every context from 6,144 to 32,768 keys on the F32 store and within 6 % on the half store (64 starves the machine; 176 splits too finely on the F32 store); at 2 KV heads (Qwen3.5-MoE-35B-A3B) 128 against the 256 a CTA budget would give is equal at 3,072 and 6,144 keys and +1.5 % at 12,288, so the target is a per-KV-head count, not a budget. Clamped to `1..=1024`; `0` or unparsable is the default. Both knobs are read once, at init, and fix the scratch and every launch together; a change after init has no effect. | Retune on another GPU with the standalone harness before changing. |
...
| `LUMEN_CONVERT_SOURCE_FIDELITY` | `false` | config (convert-time) | Preserve supported source tensor formats verbatim on non-Metal conversions (Q5_K `ssm_out`, Q6_K output head, Q4_1 layer tensors, F32 `ssm_alpha`/`ssm_beta` gates) instead of requantizing. For a K-quant source (`Q4_K_M`, `Q5_K_M`) this is a no-op on every plane the runtime serves on the artifact being written: the default already carries its Q4_K / Q5_K / Q6_K planes, the Q4_1 layers and the F32 ssm gates. What the default keeps has to be servable, so a Q6_K head whose row width is not whole superblocks or whose stored plane is not the one the header's `vocab x hidden` needs, an `ssm_out` whose GDN width is not whole blocks for its scheme, and an `ssm_alpha` / `ssm_beta` of an extent other than the one the projection reads, are converted by default as 0.31.0 converted them — and so is the `ssm_out` of a `--requant` or `--dequantize` conversion at any width, since those stamp a Q8_0 / Q4_0 / F32 header and the K-quant layer kernels are scoped on a K-quant header (the other preserved planes go by arms that read a plane's stored scheme whatever the header is: a kept embedding survives both `--requant` schemes, while `--dequantize` dequantises it; a preserved Q6_K head survives `--requant q8_0` and `--dequantize`, while `--requant q4_0` requantises it as 0.31.0 did). The flag is 0.31.0's answer on each: the head and the gates are kept as stored; the flag itself keeps an `ssm_out` only when it is Q5_K or Q8_0, and a kept Q5_K one at such a width makes the conversion fail, exactly as in 0.31.0 — a Q4_K / Q6_K / Q8_0 one converts the same way with or without the flag. For other sources the scope is as stated. | Set when byte-exact parity with the source checkpoint matters more than the fastest kernels. |
| `LUMEN_CONVERT_KEEP_Q6K_OUTPUT` | `false` | config (convert-time) | Preserve a Q6_K output head verbatim on non-Metal conversions (subset of `LUMEN_CONVERT_SOURCE_FIDELITY`). For a K-quant source (`Q4_K_M`, `Q5_K_M`) this is a no-op on every head the runtime serves: the default already keeps its Q6_K head, but a K-quant source's default head has to be servable, so a Q6_K head whose row width is not whole superblocks, or whose stored plane is not the one the header's `vocab x hidden` needs, is requantised by default and this flag preserves it. For other sources the scope is as stated. | Set for CUDA source-fidelity heads without the rest of the fidelity set. |
...
| `LUMEN_CUDA_LEGACY_DEFAULTS` | OFF (canonical defaults ON) | config | Roll every CUDA default back to its legacy value. Decode attention has one kernel and no legacy route, so the switch does not touch it. | `=1` to reproduce a pre-optimization baseline. |
...
| Variable | Default | Category | Effect | When to touch |
|---|---|---|---|---|
| `LUMEN_CUDA_ATTN_DUMP` | unset | diagnostic | `<dir>:<seq_len>[,<seq_len>...]` — at every decode-attention call whose sequence length is listed, writes `attn-<seq_len>-<call>.json` (shape, scale, the route that served with its split count, partition, policy and codegen, `kv_dtype`) beside raw little-endian files: `.q.f32` (`[num_heads, head_dim]`), the live `[num_kv_heads, seq_len, head_dim]` cache region as `.k.f32` and `.v.f32` for an F32 store or as `.k.f16` and `.v.f16` (16-bit half patterns, the bytes the kernel read) for a half store, and `.out.f32` (the route's output). The call counter runs over the process, so one token's attention layers appear in order. Copies the cache to the host on every listed call; not for a measured run. A malformed value is said once and ignored. | Replaying real activations through a float64 reference implementation to measure each decode route's error on real inputs. |
| `LUMEN_ATTN_FIXTURE_WRITE` | unset | diagnostic (tests only) | Read by `cuda_attention_decode_fixture_test` alone: `<dir>` makes the suite run the decode-attention kernel on its deterministic inputs and write every raw array beside a `manifest.json` of digests, instead of checking the kernel against the committed fixture. The engine never reads it. | Only to recapture the reference fixture from a kernel whose output is meant to become the new reference. |
...
| `LUMEN_CUDA_VERBOSE` | OFF | diagnostic | Verbose CUDA backend logging (kernel-load failures, and a one-line `ACTIVE` announcement from each decode-attention, dense decode-matvec and output-head route the first time it is dispatched, naming the CUDA symbol that launched, or the cuBLAS entry point where cuBLAS chose the kernel; MoE expert dispatches do not announce). The `[CUDA] K-quant planes:` counters line prints at load on an artifact whose header scheme is K-quant and that carries an as-stored K-quant plane (`Q4_K_M`, `Q5_K_M` on the generic target) regardless, and under `=1` on any artifact. | Diagnose kernel-load / driver issues; confirm which decode routes a run actually took. |
...
Decode attention runs one kernel pair on every model and every context, so the
switches that chose between its predecessors are gone. Setting any of these
refuses startup, naming the variable and what replaced it, rather than
starting with a configuration the operator did not choose. Any value refuses, `0` and the empty
string included: a stack that pinned a kill switch to `0` must drop the line, not the value.

| Removed | Replacement |
|---|---|
| `LUMEN_CUDA_ATTN_SPLITK`, `LUMEN_CUDA_ATTN_SPLITK_GQA6`, `LUMEN_CUDA_DECODE_TILED` | none: there is no other route to switch on or off |
| `LUMEN_CUDA_ATTN_SPLITK_CHUNK`, `LUMEN_CUDA_ATTN_SPLITK_SCALE`, `LUMEN_CUDA_ATTN_SPLITK_GQA6_MAX_CHUNKS`, `LUMEN_CUDA_DECODE_TILED_THRESHOLD` | the partition is set by `LUMEN_CUDA_ATTN_ONE_TILE` and `LUMEN_CUDA_ATTN_TARGET` |
| `LUMEN_CUDA_ATTN_SPLITK_GQA6_ONE_TILE`, `LUMEN_CUDA_ATTN_SPLITK_GQA6_TARGET` | renamed `LUMEN_CUDA_ATTN_ONE_TILE`, `LUMEN_CUDA_ATTN_TARGET` (same meaning, same default) |
| `LUMEN_CUDA_ATTN_TILED_CODEGEN` | `LUMEN_CUDA_ATTN_CODEGEN` |
...
| `LUMEN_IMAGE_LBI` | unset (endpoint absent) | config | Directory of the converted image components (`transformer.lbi`, `vae.lbi`, `text_encoder.lbi`). With `LUMEN_IMAGE_CKPT`, enables `POST /v1/images/generations` (`--features image` builds). The files are opened and checked at startup — component tensor shapes (and bf16 storage on CUDA), the VAE configuration and the tokenizer's template markers; any failure refuses to start. | Set to serve images. |
| `LUMEN_IMAGE_CKPT` | unset | config | The source checkpoint directory, for `processor/vocab.json`, `processor/merges.txt` and `processor/added_tokens.json`. Must be set together with `LUMEN_IMAGE_LBI`. | Set with `LUMEN_IMAGE_LBI`. |
| `LUMEN_FAULT_PANIC_AT` | unset | test hook | `--features fault-injection` builds only: `prefill` or `decode:<n>` panics the engine worker at that point of the first job that reaches it, once per process; the client gets the supervisor's error and the next request is served (`docs/server.md`). A malformed value refuses to start. | Recovery tests only. |
