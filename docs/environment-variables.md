...
| `LUMEN_CUDA_ATTN_SPLITK_GQA6` | OFF | config | Serves the split-K decode-attention selection with a GQA-shared pair (`attention_decode_splitk_partial_gqa6_f32` + `attention_decode_splitk_merge_gqa6_f32`): one CTA per (KV head, chunk) instead of per (query head, chunk), so each K and V row is fetched once for the whole six-query-head group rather than once per query head, every Q, K and V read is a 16-byte load, and the merge computes each chunk's rescale once instead of once per output dimension. Eligibility: six query heads per KV head (24/4, 12/2, 6/1 alike), head_dim 256, F32 KV, context at most 4,096 — every other shape keeps the per-query-head pair and the flag is a no-op there. Under the flag the split count is `ceil(seq_len / 16)`; `LUMEN_CUDA_ATTN_SPLITK_CHUNK` and `LUMEN_CUDA_ATTN_SPLITK_SCALE` then only decide whether the split-K route is entered at all. The split-K scratch grows with the finer split — about 0.75 to 6 MiB at 24 heads and head_dim 256, allocated once at init when the flag is set. Qwen3.8-27B Q4_0 on an RTX 5090: 82.5 → 85.4 tok/s at 1,024 in / 128 out, +3.5 to +3.6 % at a 1,300-token context; per layer at 1,100 keys, partial + merge, median of 200 launches, 10.0 µs against 26.3 µs warm and 16.4 µs against 49.2–50.8 µs with the working set evicted. Largest absolute error against an F64 reference 4.3e-7 (bound by wide-score-spread inputs at 4,096 keys; uniform peaks at 9.6e-8), below the 8.2e-7 of the pair it replaces — but the reduction order differs, so the outputs are a near-tie, not byte-identical, which is why the flag is off. | `=1` to A/B it at a context on a 6:1 GQA, head_dim-256 model. Under `LUMEN_CUDA_VERBOSE=1` a dispatch that declines the route names the reason once per distinct reason. |
