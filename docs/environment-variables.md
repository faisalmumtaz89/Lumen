...
| Variable | Default | Category | Effect | When to touch |
|---|---|---|---|---|
| `LUMEN_CUDA_F16_CACHE` | unset | config | Build F16 dequant caches for the Q8_0 / Q4_0 projections of full-attention layers (Q/K/V/O and the FFN gate/up/down) as well. By default only F32 projections get one, and that copy is what decode's HGEMV reads: batched prefill dequantises each weight into scratch per matmul so its arithmetic matches decode's, and never reads these caches, so a quantised projection's copy served only the decode fallback taken for input dimensions above 24576 or when the faster matvec kernels fail to load. Measured on one card and one model (RTX 5090, Qwen3.8-27B Q4_0, 2048-token context): 11.9 GB less device memory, byte-identical output, time to first token and decode rate within run-to-run noise of the same load with the copies. | Set to `1` on a card where that decode fallback is the path taken, or to restore the previous memory profile. |
