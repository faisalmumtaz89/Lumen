...
| `LUMEN_CUDA_Q4_1_NR4` | ON | kill-switch | The source-fidelity Q4_1 matvec serves four output rows per block, reading each Q8_1 input block once per four rows (the Q4_0 kernel's layout). Same per-thread block order and warp-partial order: bit-identical. | Set to `0` for the one-row kernel. |
