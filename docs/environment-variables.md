...
| `LUMEN_CUDA_ATTN_TILED_CODEGEN` | `ptx120` for a Q4_0 dense body on compute capability 12.x when NVRTC lists compute_120 (the measured cell); NVRTC's default target elsewhere | kill-switch | The NVRTC target the tiled decode-attention kernel is compiled for: `ptx80` (compute_80), `ptx120` (compute_120) or `default`. Same source and math options; the generated code differs (NVRTC 13.3: 3632 instructions at its default target vs 2520 at compute_120; +9.6 % decode at 1,300 tokens and +14.7 % at 2,600 on the RTX 5090, byte-identical, the compute_80 control null). Only this kernel takes the device target by default — the GDN kernels grow at compute_120. The dispatch announces `attention_decode_tiled_ptx120` / `_ptx80` for an explicit target. | `=default` to A/B NVRTC's default target on Blackwell; `=ptx120` / `=ptx80` to force a target. |
| `LUMEN_CUDA_NORM_CTA5_DUAL` | ON for a Q4_0 dense body on compute capability 12.x (the measured cell: source-fidelity Qwen3.8-27B Q4_0 on the RTX 5090, +4.10 % decode); OFF elsewhere (unmeasured) | kill-switch | One multi-CTA launch at the GDN input norm writes both the normalized F32 vector and the Q8_1 blocks (replacing the plain rmsnorm + fused pair; implies the CTA5 route at every fused norm site); both outputs byte-identical (device twin tests: the dual kernel at dims 2048/5120/5152, the quantization-only kernel at 2048/4096/5120). The dual launch fires only where the GDN alpha/beta projections read the F32 vector (F32 `ssm_alpha`/`ssm_beta`, as in the source-fidelity 27B artifact, or the MoE F16 path); elsewhere the flag is the CTA5 route alone. Follows `LUMEN_CUDA_LEGACY_DEFAULTS`. | `=0` to A/B the two-launch pair on Blackwell; `=1` to force it on an unmeasured capability. |
