...
| `LUMEN_CUDA_ATTN_SPLITK_CHUNK` | 128 | config | KV positions each split-K attention chunk walks; the split count is the context length divided by this, at most 32 (the scratch bound). One chunk means the tiled kernel runs instead (no merge launch). Values below 128 are raised to 128. | Sweep it when tuning attention at context on a new card; 128 beat 256 at 330 to 2.6k tokens on the RTX 5090. |
| `LUMEN_CUDA_ATTN_SPLITK_GQA` | ON | kill-switch | With the split-K pair selected, one block per (KV head, chunk) serves every query head of that KV head's GQA group from a single read of K and V, on small chunks, instead of one block per query head re-reading them. Summation order differs. | Set to `0` for the per-query-head pass. |
| `LUMEN_CUDA_ATTN_SPLITK_GQA_CHUNK` | 64 | config | KV positions per block for the GQA-group pass (32 to 128); the split count is the context divided by this, at most 128. | Sweep when tuning attention at context on a new card. |
...
| Variable | Default | Category | Effect | When to touch |
|---|---|---|---|---|
| `LUMEN_CUDA_F16_CACHE` | unset | config | Build F16 dequant caches for the quantised (Q8_0 / Q4_0) attention projections too. By default only F32 projections get one: decode reads that copy, while a quantised projection's copy served prefill alone, and prefill dequantises into scratch per matmul at the same speed (no change in time to first token at 30 or 1,300 prompt tokens, byte-identical output) for 11.9 GB less on Qwen3.8-27B Q4_0. | Set to `1` to restore the copies on a card where you have measured a prefill benefit. |
