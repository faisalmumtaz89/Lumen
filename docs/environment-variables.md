...
| `LUMEN_CUDA_VERBOSE` | OFF | diagnostic | Verbose CUDA backend logging (kernel-load failures, and a one-line `ACTIVE` announcement from each decode-attention and output-head route the first time it is dispatched). | Diagnose kernel-load / driver issues; confirm which decode routes a run actually took. |
