...
| Variable | Default | Category | Effect | When to touch |
|---|---|---|---|---|
| `LUMEN_CUDA_ATTN_SPLITK_GQA6_MAX_CHUNKS` | unset | A/B control | Restores the bounded one-tile form of the GQA-shared pair: one CTA per 16 keys up to this many chunks (clamped to 1024), the split-K scratch sized for it, and past the bound a running generation hands off to the per-query-head pair — exactly the pre-loop behaviour (`256` = v0.29.0/v0.30.0, 4,096 positions; `1024` = the r5 bound raise, 16,384). Unset, there is no bound: the loop policy above serves any context. `0` and unparsable values are unset. | Only to reproduce the old route as the A arm of a comparison. |
