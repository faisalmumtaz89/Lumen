...
| `LUMEN_CUDA_ATTN_SPLITK_GQA6_ONE_TILE` | F32 store 176, half store 256 | config | The tile count up to which the GQA-shared pair runs one tile per CTA (bit-identical to the pre-loop form); above it the CTAs walk whole tiles at the target. The defaults are the RTX 5090 sweeps' choices. F32: never slower than one tile per CTA below 176 and faster above. Half store: against a 176 bound, both on the loop's own one-tile form (five CTAs per SM either way), 256 reads −9 % at 3,200 and −7 % at 3,968 keys but +7 to +9 % at 3,600 and +8 to +18 % at 4,096 in all four sweep runs — a wash across the grid, kept at 256 pending an engine-level retune; the previous release's six-CTA one-tile kernel explains only the store's slower band against that release (see the row above). Clamped to `1..=1024`; `0` or unparsable is the default. | Retune on another GPU with the standalone harness before changing. |
