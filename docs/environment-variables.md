...
| `LUMEN_CUDA_KQUANT` | ON | kill-switch | The general Q4_K/Q5_K/Q6_K kernels that serve a K-quant artifact's planes natively (`Q4_K_M`, `Q5_K_M` sources): decode matvec, prefill dequant tile and embedding gather per scheme. `=0` refuses a K-quant artifact at load — the tensor, its scheme and the backend are named — instead of serving it through the F32 host-dequant fallback, which cannot hold a 27B on 32 GB. A Q4_0/Q8_0/BF16 artifact's occasional K-quant plane is unaffected. | `=0` only to roll back to the load refusal; there is no pre-kernel serving path to A/B against. |
...
| `LUMEN_CONVERT_KEEP_Q6K_OUTPUT` | `false` | config (convert-time) | Preserve a Q6_K output head verbatim on non-Metal conversions (subset of `LUMEN_CONVERT_SOURCE_FIDELITY`). For a K-quant source (`Q4_K_M`, `Q5_K_M`) this is a no-op: the default already carries its Q4_K / Q5_K / Q6_K planes, the Q4_1 layers and the F32 ssm gates; for other sources the scope is as stated. | Set for CUDA source-fidelity heads without the rest of the fidelity set. |
