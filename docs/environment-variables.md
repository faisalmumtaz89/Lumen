...
| `LUMEN_CUDA_FFN_DIRECT_RESIDUAL` | ON | kill-switch | The FFN down projection folds its residual into its own store (residual sibling kernel, `attn_proj` as residual, `x_gpu` as the store), eliding the `residual_add` launch and the layer-commit device copy on every dense layer. Byte-identical — the residual is the same single f32 add the separate launch performs. | `=0` to A/B the separate residual_add + commit-copy tail. |
