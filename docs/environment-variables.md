...
| `LUMEN_BENCH_TOP2` | unset | bench | Non-streaming responses carry `lumen_bench.top2`: per generated token `[token, logit, runner_up, runner_up_logit]` — the argmax and the runner-up of the logits the decode step produced, in the engine's own numbers. Implies `LUMEN_BENCH_TOKEN_IDS`. Greedy decode runs on the host-logits route while it is on (same kernels; one vocabulary-sized copy per token), so it is a measurement surface, never a production setting. | `=1` to adjudicate a greedy flip between two routes as a near-tie or a deviation from each route's own top-2 margin. |
