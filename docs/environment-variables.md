...
| `LUMEN_CONVERT_SOURCE_FIDELITY` | `false` | config (convert-time) | Preserve supported source tensor formats verbatim on non-Metal conversions (Q5_K `ssm_out`, Q6_K output head, Q4_1 layer tensors, F32 `ssm_alpha`/`ssm_beta` gates) instead of requantizing. For a K-quant source (`Q4_K_M`, `Q5_K_M`) this is a no-op except on the head: the default already carries its Q4_K / Q5_K / Q6_K planes, the Q4_1 layers and the F32 ssm gates, but a K-quant source's default head has to be servable, so a Q6_K head whose row width is not whole superblocks is requantised by default and this flag preserves it. For other sources the scope is as stated. | Set when byte-exact parity with the source checkpoint matters more than the fastest kernels. |
