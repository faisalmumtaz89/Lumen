...
| `LUMEN_CUDA_ATTN_PREFILL_SGEMM` | ON (unset follows `LUMEN_CUDA_LEGACY_DEFAULTS`) | kill-switch | How `LUMEN_CUDA_ATTN_PRECISE=3` is evaluated for prefills of 16 tokens or more on the fused Q+gate full-attention layers: two strided-batched cuBLAS F32 SGEMMs (Q·Kᵀ over each KV head's query group, then P·V) around an exact-F32 causal row softmax, in query blocks of at most 512 rows. Exact F32 like the scalar kernel it replaces (no F16 carrier), in a different evaluation order, so greedy output can differ at a near-tie. It needs one score block of `query_heads_per_kv_head × min(prompt_tokens, 512) × total_kv_positions × 4` bytes — 100 MB for a 9B 12k-token prompt, 1.07 GB at 128k — sized and allocated once per prefill before the layer loop and freed when the prefill returns; that allocation is outside the load-time fit check. `=0`, a `LUMEN_CUDA_ATTN_PRECISE` other than `3`, `LUMEN_CUDA_FORCE_SCALAR_ATTN=1`, a shorter prefill, a failed score-block allocation, or a softmax kernel that did not build all run the scalar kernel instead. | Set to `0` to bisect prefill numerics against the scalar kernel, or on a card where the score block does not fit. |
