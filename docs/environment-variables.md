...
| `LUMEN_CUDA_VERBOSE` | OFF | diagnostic | Verbose CUDA backend logging (kernel-load failures, and a one-line `ACTIVE` announcement from each decode-attention, decode-matvec and output-head route the first time it is dispatched, naming the CUDA symbol that launched — or, for the cuBLAS-backed routes, the host-side route that ran). | Diagnose kernel-load / driver issues; confirm which decode routes a run actually took. |
