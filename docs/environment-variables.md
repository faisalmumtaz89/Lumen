...
| Variable | Default | Category | Effect | When to touch |
|---|---|---|---|---|
| `LUMEN_CUDA_ATTN_SPLITK_GQA6_MAX_CHUNKS` | 1024 | config | The split-count bound the GQA-shared decode-attention pair serves: `ceil(seq_len / 16)` chunks up to this many, so 16,384 KV positions at the shipped bound, with the split-K scratch sized to the smaller of the context and the bound (24.2 MiB at the bound on a 24-head, head_dim-256 model: the output partials plus each chunk's running max and sum). Past the bound a running generation returns to the per-query-head pair. Values above 1024 clamp to 1024; `0`, negative and unparsable values fall to 1024. `256` reproduces the v0.29.0/v0.30.0 bound (4,096 positions, 6.05 MiB), the A/B arm of the promotion record. | Lower it to bound the scratch or to A/B the long-context route; nothing raises it past 1024. |
