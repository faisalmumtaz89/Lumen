...
| `LUMEN_CUDA_ATTN_SPLITK_GQA6_TARGET` | 128 | config | The split count the GQA-shared pair holds above the one-tile bound; each CTA walks `ceil(tiles / 128)` or one fewer whole tiles. 128 is within 4 % of the best measured count at every context from 6,144 to 32,768 keys on both stores on the RTX 5090 (64 starves the machine, 176+ splits too finely). Clamped to `1..=1024`. | Retune on another GPU with the standalone harness before changing. |
