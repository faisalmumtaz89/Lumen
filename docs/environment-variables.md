...
|---|---|---|---|---|
| `LUMEN_KV_PRECISION` | backend: Metal `f16`, CUDA/CPU `f32` | config | KV-cache storage (`f16`/`f32`); invalid value warns + falls back. CLI `--kv-precision` wins (the server takes the same flag). **Metal is F16-only** (`f32` rejected — GPU-resident KV buffers are allocated F16). **CUDA: `f16` is the opt-in half store.** Every attention layer's K and V cache is allocated as IEEE half; the three writers (the fused Q/K/V prep, the per-token write, the batched prefill write) round to nearest even on the way in and count every value that does not fit in half (`|x| ≥ 65,520`, or non-finite; the slot holds ±Inf or NaN), and a non-zero count refuses the generation at the next readback — after the prefill, after every decode token, after every streaming logits step — so no output computed over a poisoned slot leaves the engine; the decode readers are half twins of the F32 kernels, chosen by the same admission decision the F32 store uses (one function decides the split-K choice for both stores): `attention_decode_splitk_partial_gqa6_loop_f16` with the F32 merge where the F32 store takes the GQA-shared pair, `attention_decode_splitk_partial_f16` with the F32 merge where it takes the per-query-head pair (another GQA ratio, or past the GQA-shared bound), `attention_decode_tiled_f16` where it takes the tiled kernel; `LUMEN_CUDA_DECODE_TILED=1` forces the tiled half kernel as it forces the tiled kernel, and `LUMEN_CUDA_ATTN_SPLITK=0` returns the half store to it; the single-block route has no half twin, so a non-default `LUMEN_CUDA_DECODE_TILED_THRESHOLD` with a half store is refused at init, as is a model whose head_dim is not a multiple of 128); the widening on load is exact and the arithmetic after it is the F32 kernel's in the F32 kernel's order, so on half-representable inputs the twins reproduce the F32 kernels bit for bit; the prefill readers work on the cache widened to F32 into one shared pair of buffers allocated at init at the cache's capacity (4 × 2 × max_seq_len × head_dim × num_kv_heads bytes, named in the `[CUDA mem]` receipt; the widening itself is exact). What changes numerically is the storage rounding of K and V: on real Qwen3.8-27B activations at 5k and 11k keys the half-stored attention output differs from the F32-stored one by a relative L2 of 1.7–1.9e-4 median, 7.5e-4 worst per call (K carries about four times the error of V; the largest stored |k| was 23 and |v| 106, far from the half ceiling). Memory: 1.07 GB instead of 2.15 GB for the cache itself on Qwen3.8-27B (the prefill widening pair the half store allocates at start-up adds 128 MB at 16,384 positions, so the net saving there is 0.95 GB)'s 16 attention layers at 16,384 positions. The backend's store and the session's `kv_precision` must agree; a mismatch is refused before any cache is allocated. `LUMEN_CUDA_ATTN_DUMP` writes a half store's region as `.k.f16`/`.v.f16` with `"kv_dtype": "f16"` in the header. | `f32` for a numerical-parity check on CPU. |
...
| `LUMEN_CUDA_ATTN_TILED_CODEGEN` | `ptx120` for a Q4_0 dense body on compute capability 12.x (measured on 12.0; every 12.x part takes it) when NVRTC lists compute_120; NVRTC's default target elsewhere | kill-switch | The NVRTC target the tiled decode-attention kernel is compiled for: `ptx80` (compute_80), `ptx120` (compute_120) or `default`. Same source and math options; the generated code differs (NVRTC 13.3: 3632 instructions at its default target vs 2520 at compute_120; +9.6 % decode at 1,300 tokens and +14.7 % at 2,600 on the RTX 5090, byte-identical, the compute_80 control null). Only this kernel takes the device target by default — the GDN kernels grow at compute_120. The dispatch announces `attention_decode_tiled_ptx120` / `_ptx80` for an explicit target. | `=0` (or `=default`) to A/B NVRTC's default target on Blackwell; `=ptx120` / `=ptx80` to force a target (a target the toolkit or driver refuses falls back to the default target, announced). |
| `LUMEN_CUDA_NORM_CTA5_DUAL` | ON for a Q4_0 dense body on compute capability 12.x (measured on 12.0: source-fidelity Qwen3.8-27B Q4_0 on the RTX 5090, +4.10 % decode; every 12.x part takes it); OFF elsewhere (unmeasured) | kill-switch | One multi-CTA launch at the GDN input norm writes both the normalized F32 vector and the Q8_1 blocks (replacing the plain rmsnorm + fused pair; the multi-CTA `rmsnorm_to_q8_1_cta5` route at every fused norm site comes with it); both outputs byte-identical (device twin tests: the dual kernel at dims 2048/4096/5120/5152, the quantization-only kernel at 2048/4096/5120). The dual launch fires only where the GDN alpha/beta projections read the F32 vector (F32 `ssm_alpha`/`ssm_beta`, as in the source-fidelity 27B artifact, or the MoE F16 path); elsewhere the flag is the CTA5 route alone. Follows `LUMEN_CUDA_LEGACY_DEFAULTS`. | `=0` to A/B the two-launch pair on Blackwell; `=1` to force it on an unmeasured capability. |
...
| `LUMEN_CUDA_SOA_LOCKED` | ON for Q4-dense on cc 8.x/9.x only; OFF on every other capability (cc 12.x Blackwell, cc 10.x, or a failed capability query) and OFF for MoE/BF16 | kill-switch | Codegen-locked SoA Q4 matvec (word-load reorder; +30% on A100/H100). On an RTX 5090, in a configuration that fits, forcing it on does not reproduce the 1.1 tok/s once attributed to it (75.4–75.5 tok/s either way); it defaults off there because the kernel is unmeasured on that capability, not because it is slow. Turning it on also enables the Q4 split dispatch. | `=0` to A/B the AoS Q4 matvec; `=1` to force it on an unmeasured capability. |
...
| `LUMEN_CUDA_ATTN_SPLITK` | ON for Q8_0- and BF16-body dense models on every capability, and for Q4_0-body dense models on compute capability 12.x (measured on 12.0; every 12.x part takes it; model- and device-aware) | kill-switch | Routes full-attention decode through a split-K kernel pair (sequence-parallel: one CTA per query head per chunk + merge) when the auto-selection would choose the tiled kernel — lifts the one-CTA-per-head occupancy ceiling on few-head models by splitting each head's KV walk across several partial CTAs plus a merge pass (total work stays linear in context length). The chunk count follows the context length (`LUMEN_CUDA_ATTN_SPLITK_CHUNK`); at a count of 1 the tiled kernel runs. Quality-equivalent near-tie (cross-chunk merge order, which follows the chunk count), DET+GQ gate-banked at a fixed count of 4. Precedence: explicit `LUMEN_CUDA_DECODE_TILED=1` forces the Tiled base selection (subject to the existing incompatible-head-dimension fallback), and a SingleBlock threshold opt-out is untouched; follows `LUMEN_CUDA_LEGACY_DEFAULTS`. | `=0` to A/B the tiled one-CTA-per-head route; `=1` to force the pair on a Q4 body on a capability it is unmeasured on. On the RTX 5090 the Q4_0 27B pair is the difference between 60.7 and 79.4 tok/s at 1,024 tokens of context (the tiled kernel collapses there: one CTA per head on a 170-SM card). |
| `LUMEN_CUDA_ATTN_SPLITK_SCALE` | ON (canonical) | kill-switch | Sizes the split-K attention split count from the context instead of the fixed 4 the pair shipped with through v0.24.0. The merge sums a different number of chunks, so on the classes that take the pair by default it is a near-tie numerics change, not byte-identical. | `=0` to A/B the fixed 4-chunk route (the DET+GQ gate-banked configuration). While the GQA-shared pair is active (`LUMEN_CUDA_ATTN_SPLITK_GQA6`, on by default for a Q4_0 dense body on compute capability 12.x) on a shape it serves (six query heads per KV head, head_dim 256, context within its chunk bound), this chooses only whether the split-K route is entered; that route's own split count is `ceil(seq_len / 16)` up to its one-tile bound and the fixed target (128) above it. |
| `LUMEN_CUDA_ATTN_SPLITK_CHUNK` | 128 | config | Target KV positions per split-K attention chunk. It sets the chunk count — the context length divided by this, rounded up, at most 32 (the scratch bound) — and the context is then split evenly into that many chunks, so the span one chunk walks is this value or less until the cap binds (65 and 64 at a context of 129, 119 at 1300; 384 at 12280, where the count is capped at 32). A count of 1 means the tiled kernel runs instead (no merge launch) when it loaded; otherwise the pair runs with one chunk. Values below 128 are raised to 128 and an unparseable value is the default; either substitution is printed once. Not read under `LUMEN_CUDA_ATTN_SPLITK_SCALE=0`. | Sweep it when tuning attention at context on a new card; 128 beat 256 at 330 to 2.6k tokens on the RTX 5090. While the GQA-shared pair is active (`LUMEN_CUDA_ATTN_SPLITK_GQA6`, on by default for a Q4_0 dense body on compute capability 12.x) on a shape it serves (six query heads per KV head, head_dim 256, context within its chunk bound), this chooses only whether the split-K route is entered; that route's own tile is fixed at 16 keys (one tile per CTA up to its one-tile bound, a run of whole tiles per CTA above it). |
| `LUMEN_CUDA_ATTN_SPLITK_GQA6` | ON for a Q4_0 dense body on compute capability 12.x (measured on 12.0: source-fidelity Qwen3.8-27B Q4_0 on the RTX 5090; every 12.x part takes it); OFF elsewhere (unmeasured); follows `LUMEN_CUDA_LEGACY_DEFAULTS` | kill-switch | Serves the split-K decode-attention selection with a GQA-shared pair (`attention_decode_splitk_partial_gqa6_loop_f32` + `attention_decode_splitk_merge_gqa6_f32`): one CTA per (KV head, chunk) instead of per (query head, chunk), so each K and V row is fetched once for the whole six-query-head group rather than once per query head, every Q, K and V read is a 16-byte load, and the merge computes each chunk's rescale once instead of once per output dimension. Eligibility: six query heads per KV head (24/4, 12/2, 6/1 alike), head_dim 256, any context the cache holds — every other shape keeps the per-query-head pair and the setting is a no-op there. The partial pass is a tile loop: up to `LUMEN_CUDA_ATTN_SPLITK_GQA6_ONE_TILE` tiles of 16 keys (176, on either store) each CTA takes one tile, the form every release since v0.29.0 took and bit-identical to it; above that the split count is held at `LUMEN_CUDA_ATTN_SPLITK_GQA6_TARGET` CTAs (128) and each walks a balanced run of whole tiles with the tiled kernel's running-max recurrence, so the split-K scratch is 4.2 MiB at any context on a 24-head model (either store) and a generation never changes route as it grows (`LUMEN_CUDA_ATTN_SPLITK_GQA6_MAX_CHUNKS` restores the old bounded form as an A/B control). Measured on the RTX 5090 against the one-tile form (evicted-L2 medians per layer): F32 −7.6 % at 2,600 keys, −12 % at 3,200–3,968, −17 % at 4,096, −19 to −21 % at 6,144, −21 % at 12,288, −27 % at 16,384, never slower at any measured context; half store −10 % at 2,600, equal at 2,816 and 3,200, −3 to −8 % at 3,600, +17 % at 3,968 in every run (the one slower cell: the loop's 128 CTAs per KV head each walk two tiles in series where the pre-loop one-tile form's 992 CTAs fit one wave at six per SM), −8 to −15 % at 4,096, −7 % at 4,800, −17 % at 6,144, −27 % at 12,288, −29 % at 16,384, the sole exception elsewhere one timer tick (+0.2 %) at 1,152 keys in one run of four (the harness timer is quantised near 2 µs, so the cells are coarse); and 141 / 178 µs per layer (F32) at 24,576 / 32,768 keys, which the bounded form could not serve. Below the one-tile bound the split count is `ceil(seq_len / 16)`; `LUMEN_CUDA_ATTN_SPLITK_CHUNK` and `LUMEN_CUDA_ATTN_SPLITK_SCALE` then only decide whether the split-K route is entered at all. The split-K scratch is sized once at init for the policy's larger count (the output partials plus each chunk's running max and sum: 4.2 MiB at 176 chunks on 24 heads and head_dim 256, either store), never for the context; under the A/B control it is sized for that bound (24.2 MiB at 1,024). Qwen3.8-27B Q4_0 on an RTX 5090, the promoted build against itself under `=0`: 82.59 → 85.56 tok/s at 1,024 in / 128 out, 83.86 → 85.35 at 21 in / 257 out, 80.28 → 83.60 at 3,072 in / 128 out (82.5–82.6 → 85.4–85.6 at 1,024 / 128 across five sessions on successive builds of the pair); +3.5 to +3.6 % at a 1,300-token context; per layer at 1,100 keys, partial + merge, median of 200 launches, 10.0 µs against 26.3 µs warm and 16.4 µs against 49.2–50.8 µs with the working set evicted. Largest absolute error against an F64 reference on synthetic inputs under 2e-6 across 1..16,384 keys (the suite prints the observed maximum per length); on real activations at 5k and 11k keys the pair's worst coordinate error is 7.0e-6 and 2.2e-5 against 2.0e-5 and 3.0e-5 for the per-query-head pair — but the reduction order differs, so the outputs are a near-tie with the per-query-head pair, not byte-identical (the greedy stream on the 1,024-token board prompt parts at token 3 on a mutual-runner-up flip with margins of 0.028 and 0.035 logits; the 1,335-token context fixture is identical). With the pair serving every context, the 54-prompt long-context record (1.9k–15.2k tokens) against the tiled reference is 38 identical / 15 near-ties / 1 deviation (a 3,129-token continuation parting at token 43, where the pair's own top two logits are 0.004 apart — its runner-up is the reference's token — against the reference's margin 0.070), needle retrieval 35/36 on every arm; that deviation was replayed through a float64 reference on the real activations at the parting positions (3,169–3,176 keys, all 16 attention layers): the pair's worst relative L2 3.8e-7 (worst coordinate 1.3e-5) against 9.3e-7 (3.5e-5) for the tiled reference, so the pair meets the declared numerical envelope at the parting positions and the divergence is the model amplifying a float32-precision difference; the record's rule keeps it on file as a deviation. | `0`/`off`/`false`/`no` return the measured cell to the per-query-head pair (the pair is not loaded and the scratch stays at its smaller size); `1`/`on`/`true`/`yes` force it on wherever the pair is eligible, including unmeasured capabilities; any other spelling falls to the default; spellings are matched case-insensitively. The startup `capability-keyed routes` line reports what is in force (for this pair, together with `LUMEN_CUDA_ATTN_SPLITK`, which the loader requires). Under `LUMEN_CUDA_VERBOSE=1` a dispatch that declines the route names the reason once per distinct reason. |
...
| `LUMEN_CUDA_Q4_SPLIT` | armed by `SOA_LOCKED` for Q4-dense on cc 8.x/9.x; otherwise off unless set | kill-switch | Raw+split layout for Q4_0 weights (+9.0% Q4 decode); load-bearing under SoA-lock. | Leave as-is; the env read is A/B-only. |
...
| `LUMEN_CUDA_Q4_SPLIT_BUDGET_GB` | auto (free-mem-aware) | config | The Q4 SoA split-clone budget for the dense-FFN decode kernel. Default is resolved at preload as `free_mem - 2 GB activation slack`, where `free_mem` is read after `init()` has allocated the KV cache (so it is already net of KV). | `=N` (GB) to pin the budget; the value is used as given and is not capped. |
| `LUMEN_CUDA_Q8_SPLIT_BUDGET_GB` | auto (free-mem-aware) | config | The Q8 SoA split-clone budget for the dense-FFN decode kernel. Default is resolved at preload as `free_mem - 2 GB activation slack`, where `free_mem` is read after `init()` has allocated the KV cache (so it is already net of KV). | `=N` (GB) to pin the budget; the value is used as given and is not capped. |
...
| Variable | Default | Category | Effect | When to touch |
|---|---|---|---|---|
| `LUMEN_CUDA_ATTN_SPLITK_GQA6_MAX_CHUNKS` | unset | A/B control | Restores the bounded one-tile form of the GQA-shared pair by launching the previous release's partial (`attention_decode_splitk_partial_gqa6_f32`, kept compiled) and its half twin (`_f16`, new in this release): one CTA per 16 keys up to this many chunks (clamped to 1024), the split-K scratch sized for it, and past the bound a running generation hands off to the per-query-head pair — the pre-loop partition, route boundary and partial kernels, on this release's eight-lane merge (`256` = the v0.29.0/v0.30.0 bound, 4,096 positions; `1024` = this release's bound raise, 16,384). Unset, there is no bound: the loop policy above serves any context. `0` and unparsable values are unset. | Only to reproduce the old route as the A arm of a comparison. |
| `LUMEN_CUDA_ATTN_SPLITK_GQA6_ONE_TILE` | 176 | config | The tile count up to which the GQA-shared pair runs one tile per CTA (bit-identical to the pre-loop form); above it the CTAs walk whole tiles at the target. The default, one bound on either store, is the RTX 5090 sweeps' choice: on the F32 store the loop is never slower than one tile per CTA below 176 and faster above; on the half store 176 against 256 is a wash on the fine grid (176 reads +10 % at 3,200 keys and +8 % at 3,968, −6 to −8 % at 3,600 and −8 to −15 % at 4,096, equal elsewhere, over the four runs), and one bound gives both stores the same split geometry at every context, so on half-representable inputs the half store's output is bit-identical to the F32 store's. Clamped to `1..=1024`; `0` or unparsable is the default. | Retune on another GPU with the standalone harness before changing. |
| `LUMEN_CUDA_ATTN_SPLITK_GQA6_TARGET` | 128 | config | The split count the GQA-shared pair holds above the one-tile bound; each CTA walks `ceil(tiles / 128)` or one fewer whole tiles. At every context from 6,144 to 32,768 keys, 128 is within 3 % of the best measured count on the F32 store and within 6 % on the half store (the gap at 32,768 keys, where 176 is best) on the RTX 5090 (64 starves the machine; 176 splits too finely on the F32 store at every context — the slowest F32 count, +23 % at 16,384 and 32,768 — and is the best half-store count from 16k up). Clamped to `1..=1024`. | Retune on another GPU with the standalone harness before changing. |
| `LUMEN_CUDA_F16_CACHE` | unset | config | Build F16 dequant caches for the Q8_0 / Q4_0 projections of full-attention layers (Q/K/V/O and the FFN gate/up/down) as well. By default a projection gets one when it is F32, or when it is a K/V beside an F32 Q or an up beside an F32 gate: those are the copies decode's HGEMV reads. Batched prefill dequantises each weight into scratch per matmul so its arithmetic matches decode's, and never reads these caches, so the other quantised copies served only the decode fallback taken for input dimensions above 24576 or for a matvec kernel that failed to load. Measured on one card and one model (RTX 5090, Qwen3.8-27B Q4_0, 2048-token context): 11.9 GB less device memory, byte-identical output, time to first token and decode rate within run-to-run noise of the same load with the copies. | Set to `1` on a card where that decode fallback is the path taken, or to restore the previous memory profile. |
| `LUMEN_CUDA_F16_CACHE_FORCE` | unset | config | Two guards read it. The F16 dequant caches (a full-attention layer's F32 projections and the quantised K/V or up beside an F32 Q or gate, plus its other quantised ones under `LUMEN_CUDA_F16_CACHE`) are refused before they are built when their bytes plus a 128 MiB margin exceed free memory (a failed free-memory query builds them: a telemetry failure is not a memory shortage, and a load that got this far is refused only on a measured one); the output-projection split clone is skipped when it would leave less than the 2 GB decode slack or when the free-memory query fails (it is an optimisation). `=1` makes both go ahead regardless. | `=1` when a load is refused, or a clone skipped, that you know fits. |
...
| `LUMEN_CUDA_ATTN_PREFILL_SGEMM` | ON (unset follows `LUMEN_CUDA_LEGACY_DEFAULTS`) | kill-switch | How the exact-F32 prefill attention is evaluated for prefills of 16 tokens or more on the fused Q+gate full-attention layers: two strided-batched cuBLAS F32 SGEMMs (Q·Kᵀ over each KV head's query group, then P·V) around an exact-F32 causal row softmax, in query blocks of at most 512 rows. Exact F32 like the scalar kernel it replaces (no F16 carrier), in a different evaluation order, so greedy output can differ at a near-tie. It needs one score block of `query_heads_per_kv_head × min(prompt_tokens, 512) × total_kv_positions × 4` bytes — 100 MB for a 9B 12k-token prompt, 1.07 GB at 128k — sized and allocated once per prefill before the layer loop and freed when the prefill returns; that allocation is outside the load-time fit check. With `=0`, `LUMEN_CUDA_FORCE_SCALAR_ATTN=1`, a shorter prefill, a refused score-block allocation, or a softmax kernel that did not build, the attention runs on the scalar kernel instead. | Set to `0` to bisect prefill numerics against the scalar kernel, or on a card where the score block does not fit. |
...
| `LUMEN_CUDA_CT4_EXACTK` | ON | perf | Launch the CtInt4G32 decode matvec with a block size matched to the reduction depth: the K=5120 / K=6144 shapes (160 / 192 g32 blocks per row) use 160- / 192-thread kernels instead of 256, removing wholly idle warps with bit-identical output. K=17408 keeps 256. `=0` opts out (escape hatch). | Shipping occupancy route for imported ct4 checkpoints (measured +11.3% decode on Qwen3.8-27B ct4, A100); `=0` restores the fixed-256 launch for A/B attribution. |
...
| Variable | Default | Category | Effect | When to touch |
|---|---|---|---|---|
| `LUMEN_CUDA_ATTN_DUMP` | unset | diagnostic | `<dir>:<seq_len>[,<seq_len>...]` — at every decode-attention call whose sequence length is listed, writes `attn-<seq_len>-<call>.json` (shape, scale, the route that served, `kv_dtype`) beside raw little-endian files: `.q.f32` (`[num_heads, head_dim]`), the live `[num_kv_heads, seq_len, head_dim]` cache region as `.k.f32` and `.v.f32` for an F32 store or as `.k.f16` and `.v.f16` (16-bit half patterns, the bytes the kernel read) for a half store, and `.out.f32` (the route's output). The call counter runs over the process, so one token's attention layers appear in order. Copies the cache to the host on every listed call; not for a measured run. A malformed value is said once and ignored. | Replaying real activations through a float64 reference implementation to measure each decode route's error on real inputs. |
...
| `LUMEN_CUDA_VERBOSE` | OFF | diagnostic | Verbose CUDA backend logging (kernel-load failures, and a one-line `ACTIVE` announcement from each decode-attention, dense decode-matvec and output-head route the first time it is dispatched, naming the CUDA symbol that launched, or the cuBLAS entry point where cuBLAS chose the kernel; MoE expert dispatches do not announce). | Diagnose kernel-load / driver issues; confirm which decode routes a run actually took. |
| `LUMEN_CUDA_GDN_SUBSTAGE_TIMING` | OFF | diagnostic | Per-substage GDN timing. | Attribute time inside the GDN block. |
| `LUMEN_CUDA_FORCE_SCALAR_ATTN` | OFF | diagnostic | Force the scalar attention kernel. | Compare the tiled and scalar evaluations of the exact-F32 prefill attention. |
...
| `LUMEN_BENCH_TOP2` | unset | bench | Non-streaming responses carry `lumen_bench.top2`: per generated token `[argmax, logit, runner_up, runner_up_logit]` — the argmax and the runner-up of the logits as the session received them from the backend, before the host-side EOG mask, penalties and sampling (the CUDA decode path masks EOG on the device before readback) (the selected token is `generated_token_ids[i]`; under greedy decode with penalties off and no mask they coincide, which is the configuration the surface is for). Implies `LUMEN_BENCH_TOKEN_IDS`. Greedy decode runs on the host-logits route while it is on (same kernels; one vocabulary-sized copy per token), so it is a measurement surface, never a production setting. | `=1` to adjudicate a greedy flip between two routes as a near-tie or a deviation from each route's own top-2 margin. |
