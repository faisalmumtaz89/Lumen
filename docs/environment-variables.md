...
| `LUMEN_BENCH_TOP2` | unset | bench | Non-streaming responses carry `lumen_bench.top2`: per generated token `[argmax, logit, runner_up, runner_up_logit]` — the argmax and the runner-up of the RAW logits the decode step produced, before the EOG mask, penalties and sampling (the selected token is `generated_token_ids[i]`; under greedy decode with penalties off and no mask they coincide, which is the configuration the surface is for). Implies `LUMEN_BENCH_TOKEN_IDS`. Greedy decode runs on the host-logits route while it is on (same kernels; one vocabulary-sized copy per token), so it is a measurement surface, never a production setting. | `=1` to adjudicate a greedy flip between two routes as a near-tie or a deviation from each route's own top-2 margin. |
