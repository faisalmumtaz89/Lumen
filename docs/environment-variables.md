...
| Variable | Default | Category | Effect | When to touch |
|---|---|---|---|---|
| `LUMEN_CUDA_ATTN_SPLITK_GQA6_MAX_CHUNKS` | unset | A/B control | Restores the bounded one-tile form of the GQA-shared pair by launching the previous release's partials (`attention_decode_splitk_partial_gqa6_f32` / `_f16`, kept compiled): one CTA per 16 keys up to this many chunks (clamped to 1024), the split-K scratch sized for it, and past the bound a running generation hands off to the per-query-head pair — exactly the pre-loop behaviour and its kernels (`256` = v0.29.0/v0.30.0, 4,096 positions; `1024` = this release's bound raise, 16,384). Unset, there is no bound: the loop policy above serves any context. `0` and unparsable values are unset. | Only to reproduce the old route as the A arm of a comparison. |
| `LUMEN_CUDA_ATTN_SPLITK_GQA6_ONE_TILE` | 176 | config | The tile count up to which the GQA-shared pair runs one tile per CTA (bit-identical to the pre-loop form); above it the CTAs walk whole tiles at the target. The default, one bound on either store, is the RTX 5090 sweeps' choice: on the F32 store the loop is never slower than one tile per CTA below 176 and faster above; on the half store 176 against 256 is a wash on the fine grid (176 reads +10 % at 3,200 keys and +8 % at 3,968, −8 % at 3,600 and −8 to −15 % at 4,096, equal elsewhere, all four runs), and one bound gives both stores the same split geometry at every context, so on half-representable inputs the half store's output is bit-identical to the F32 store's. Clamped to `1..=1024`; `0` or unparsable is the default. | Retune on another GPU with the standalone harness before changing. |
