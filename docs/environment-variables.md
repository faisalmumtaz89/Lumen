...
| `LUMEN_CUDA_ROPE_TAB` | ON | kill-switch | NeoX RoPE reads cos/sin from a per-CTA table computed once instead of per-thread transcendentals. Byte-identical. | `=0` to A/B per-thread angle computation. |
