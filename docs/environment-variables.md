...
| `LUMEN_CUDA_NORM_CTA5_DUAL` | OFF | experiment | One multi-CTA launch at the GDN input norm writes both the normalized F32 vector and the Q8_1 blocks (replacing the plain rmsnorm + fused pair; implies the CTA5 route at every fused norm site); both outputs byte-identical. | `=1` to A/B against the two-launch pair. |
