...
| `LUMEN_CUDA_Q4_SPLIT_BUDGET_GB` | auto (free-mem-aware) | config | Upper cap on VRAM used for Q4 SoA split siblings. The split layout itself is unconditional — this sizes it, it does not switch it. Default is resolved at preload as `free_mem − 2 GB activation slack`; the KV cache is already allocated by then, so it is not subtracted again. FFN, attention and GDN projections are all eligible, largest-first. | `=0` clones nothing (every projection stays on its base kernel); `=N` requests an N-GB cap, clamped to the resource-aware default so an over-large request cannot push preload into OOM; a negative or non-numeric value is a startup error, never a silent fallback to auto. |
| `LUMEN_CUDA_Q8_SPLIT_BUDGET_GB` | auto (free-mem-aware) | config | The Q8 analog of the row above, resolved by the same shared resolver (`free_mem − 2 GB activation slack`, no KV re-reservation). On 27B-Q8 the resolved cap exceeds the ~19 GB FFN so all FFN layers clone onto the codegen-locked SoA kernel (+16.5% 27B-Q8 decode; quality-equivalent benign near-tie, DET+GQ gate-banked). No-op on MoE. | Same semantics as the row above: `=0` clones nothing, `=N` is clamped to the safe cap, malformed values are a startup error. |
...
| Variable | Default | Category | Effect | When to touch |
|---|---|---|---|---|
| `LUMEN_CUDA_PROFILE` | OFF | diagnostic | Per-phase decode profiling via CUDA-event brackets; prints one `[PROFILE]` block per `generate` call to stderr. `1` = block-level phases only (embed / gdn_attn / full_attn / ffn / layer_commit / head / argmax). `2` = adds the coarse groups AND per-surface leaves: every matvec (`attn_wq_q6k`, `attn_wq_q4`, `attn_wkv_q6k`, `attn_wkv_q4`, `attn_wo_mv`, `ffn_gate_mv`, `ffn_up_mv`, `ffn_down_mv`, `gdn_qkv_mv`, `gdn_gate_mv`, `gdn_alpha_mv`, `gdn_beta_mv`, `ssm_out_mv`, `head_mv`) with `bytes_per_token` and `eff_gbps`, each activation quantize timed separately and netted out of its matvec, plus `gdn_conv`/`gdn_recur` split and per-parent `uncovered` residuals. `cupti` = leave this off and use `tools/cupti-inject`. Adds no synchronization. | Attribute decode time to a surface and read its effective bandwidth. Level 1 for a cheap block-level split; level 2 to ask whether a byte reduction converts into time. Overhead of both levels is UNMEASURED — A/B off vs 1 vs 2 before quoting absolutes. Do not combine with `LUMEN_XCHK` or `LUMEN_MOE_PROBE`. |
