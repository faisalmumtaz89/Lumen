...
| `LUMEN_CUDA_ATTN_PREFILL_SGEMM` | ON | kill-switch | Prefills of 16 tokens or more run attention as tiled cuBLAS F32 SGEMM (Q·Kᵀ and P·V, strided-batched over each KV head's query group) around an exact-F32 causal softmax, in query blocks of 512 rows; exact F32 like the scalar kernel, a different summation order. Off, the `LUMEN_CUDA_ATTN_PRECISE` selection applies. | Set to `0` to bisect prefill numerics against the scalar kernel. |
