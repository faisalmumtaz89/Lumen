...
| `LUMEN_CUDA_ATTN_TILED_CODEGEN` | unset (NVRTC default target) | experiment | Compile the tiled decode-attention kernel for an explicit NVRTC target: `ptx80` (compute_80) or `ptx120` (compute_120). Same source and math options; the generated code differs (NVRTC 13.3: 3632 instructions at its default target vs 2520 at compute_120). The dispatch announces `attention_decode_tiled_ptx80` / `_ptx120`. | `=ptx120` / `=ptx80` to A/B the code generation. |
| `LUMEN_CUDA_NORM_CTA5_DUAL` | OFF | experiment | One multi-CTA launch at the GDN input norm writes both the normalized F32 vector and the Q8_1 blocks (replacing the plain rmsnorm + fused pair; implies the CTA5 route at every fused norm site); both outputs byte-identical. The dual launch fires only where the GDN alpha/beta projections read the F32 vector (F32 `ssm_alpha`/`ssm_beta`, as in the source-fidelity 27B artifact, or the MoE F16 path); elsewhere the flag is the CTA5 route alone. | `=1` to A/B against the two-launch pair. |
...
| `LUMEN_CUDA_RMSNORM_Q8_CTA5` | OFF | experiment | Fused RMSNorm+Q8_1 quantization over ceil(dim/32 ÷ warps) CTAs (5 at dim 5120) instead of one block; each CTA repeats the reduction, output byte-identical. | `=1` to A/B against the single-block kernel. |
...
| `LUMEN_CUDA_ATTN_SPLITK` | ON for Q8_0- and BF16-body dense models (model-aware) | kill-switch | Routes full-attention decode through a split-K kernel pair (sequence-parallel: one CTA per query head per chunk + merge) when the auto-selection would choose the tiled kernel — lifts the one-CTA-per-head occupancy ceiling on few-head models by splitting each head's KV walk across several partial CTAs plus a merge pass (total work stays linear in context length). The chunk count follows the context length (`LUMEN_CUDA_ATTN_SPLITK_CHUNK`); at a count of 1 the tiled kernel runs. Quality-equivalent near-tie (cross-chunk merge order, which follows the chunk count), DET+GQ gate-banked at a fixed count of 4. Precedence: explicit `LUMEN_CUDA_DECODE_TILED=1` forces the Tiled base selection (subject to the existing incompatible-head-dimension fallback), and a SingleBlock threshold opt-out is untouched; follows `LUMEN_CUDA_LEGACY_DEFAULTS`. | `=0` to A/B the tiled one-CTA-per-head route. |
