...
| Qwen3.5-MoE-35B-A3B | Q4_0 | 213.5 / 208.1 / 197.8 at 3,072 / 6,144 / 12,288 (the previous route: 80.2 / 49.8 / 28.3) | a paired gate against the previous route (+166 / +318 / +599 %), DET-001 50/50 on both stores, the F32 store identical to the previous route's at 6,153 keys; the long-context quality record (36 items) fails its strict first-flip rule on one prompt: the F32 store parts from the previous build's at token 25, where the runner-up is the same token on both builds at margins of 0.033 and 0.024 logits but is not the other's choice; the 16-bit store never parts from the previous build's 16-bit route outside the near-tie margin. Adjudicated by a float64 replay of the real activations at the parting positions: this kernel 4.7e-7 relative L2 on the F32 store against the previous route's 6.3e-7 (the 16-bit route 2.9e-7 against a float64 reference on the half-rounded inputs it read). The verification below was re-run at 128 target chunks per KV head, the shipped value |
