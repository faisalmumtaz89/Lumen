...
| K-quants on Metal, Q2_K / Q3_K, and MoE K-quant planes, at runtime | A K-quant source's Q4_K / Q5_K / Q6_K planes are served natively on CUDA (the status row above; `docs/lbc-format.md`). A Q2_K / Q3_K plane keeps the Q8_0 upcast on Metal and the host dequant at load on CUDA; the Metal target upcasts every K-quant layer plane and dequantises a K-quant embedding to F32, and any other source's K-quant plane converts as before: `--target metal` upcasts K-quant (and legacy Q5_0) layer tensors to Q8_0 and re-quantizes Q4_1 to Q4_0, unless an explicit `--requant`/`--dequantize` overrides; generic/CUDA conversions carry the plane and dequantize it at load, with the fidelity-preserved Q5_K `ssm_out` and Q6_K head on their dedicated CUDA kernels; MoE shared-expert planes are always requantized to Q4_0. | The Metal backend has no K-quant kernels; Q2_K / Q3_K and MoE K-quant planes have none on either backend |
