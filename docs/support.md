...
| Qwen3.5-9B dense | Q4_0 | 171.6 / 167.1 / 159.9 at 3,072 / 6,144 / 12,288 | a paired gate against the previous route (+3.8 / +5.9 / +10.9 %, two runs); DET-001 50/50 on both stores identical to the previous route's at 6,153 keys; in the long-context quality record the F32 store never parts from the previous build's, the 16-bit store parts from the previous build's 16-bit route once, at 12k keys, at a margin of 0.03 logits |
| Qwen3.5-MoE-35B-A3B | Q4_0 | 213.5 / 208.1 / 197.8 at 3,072 / 6,144 / 12,288 (the previous route: 80.2 / 49.8 / 28.3) | a paired gate against the previous route (+166 / +318 / +599 %), DET-001 50/50 on both stores, the F32 store identical to the previous route's at 6,153 keys; the long-context quality record parts from the previous build once (both stores, one prompt, a three-way tie of 0.05 logits), adjudicated by a float64 replay of the real activations at the parting positions (this kernel 4.7e-7 relative L2 on the F32 store and 2.9e-7 on the 16-bit store, against the previous route's 6.3e-7) |
